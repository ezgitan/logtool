...
public sealed class SetupController : ControllerBase
{
    ...
    private static string BuildScript(string? certBase64, string siteUrl)
    {
        ...
        sb.AppendLine("    On Error Goto 0");
        sb.AppendLine("    LogMsg \"Admin access step finished (approval screen closed). Waiting for the grant to take effect...\"");
        sb.AppendLine();
        sb.AppendLine("    ' The approval screen closing doesn't mean the access is active yet -");
        sb.AppendLine("    ' retry a few times with a short wait instead of checking only once.");
        sb.AppendLine("    Dim attempt");
        sb.AppendLine("    For attempt = 1 To 4");
        sb.AppendLine("        WScript.Sleep 3000");
        sb.AppendLine("        upn = GetUpn()");
        sb.AppendLine("        LogMsg \"whoami retry \" & attempt & \" result: '\" & upn & \"'\"");
        sb.AppendLine("        If upn <> \"\" Then Exit For");
        sb.AppendLine("    Next");
        sb.AppendLine("End If");
        sb.AppendLine();
        sb.AppendLine("If upn = \"\" Then");
